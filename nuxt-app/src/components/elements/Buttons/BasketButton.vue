...
const toggleBasket = (productId: string) => {
  ...
  const index = baskets.value.indexOf(productId);
  if (index === -1) {
...
<template>
  <button @click="toggleBasket(props.productId)" aria-label="Добавить в корзину">
    <i>
      <svg width="19" height="18" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd"
              ...
              :fill="isBasket(props.productId) ? 'red' : 'black'"
        />
      </svg>
    </i>
  </button>
